...
import { User, Goal, LearnerProfile, Pathway, Quiz, Achievement } from "../models/index.js";
...
router.post("/database/:operation", adminAuth, async (req, res) => {
  try {
    ...
    logger.info(`Admin ${req.user.email} initiated database operation: ${operation}`);
    ...
    switch (operation) {
      ...
      case "populate":
        ...
        await execAsync(`node ${path.join(scriptsPath, "populateInitialData.js")}`);
        await execAsync(`node ${path.join(scriptsPath, "populateAchievements.js")}`);
        await execAsync(`node ${path.join(scriptsPath, "populateCollaborativeData.js")}`);
      ...
      case "backup":
        ...
        await execAsync(`node ${path.join(scriptsPath, "exportDatabase.js")} export`);
...
router.get("/export", adminAuth, async (req, res) => {
  try {
    ...
    logger.info(`Admin ${req.user.email} requested data export in ${format} format`);

    // Gather all data
    const [users, goals, profiles, pathways, quizzes, achievements] = await Promise.all([
    ...
    if (format === "csv") {
      ...
      res.setHeader("Content-Disposition", `attachment; filename=platform_data_${new Date().toISOString().split('T')[0]}.csv`);
      
      let csv = "Type,Email,Role,Status,Created,LastLogin\n";
      users.forEach(user => {
        csv += `User,"${user.email}","${user.role}","${user.isActive ? 'Active' : 'Inactive'}","${user.createdAt}","${user.lastLogin || 'Never'}"\n`;
      });
      ...
      res.setHeader("Content-Disposition", `attachment; filename=platform_data_${new Date().toISOString().split('T')[0]}.json`);
      res.json(exportData);
    } else {
      // JSON format
      res.setHeader("Content-Type", "application/json");
  ...
  try {
    ...
    const stats = {
      ...
      pathways: {
        ...
        completionRate: totalPathways > 0 ? Math.round((completedPathways / totalPathways) * 100) : 0,
      },
      quizzes: {
        total: totalQuizzes,
...
router.get("/health", adminAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Erreur lors de la vérification du système" });
  }
});
